...
def main():
    parser = argparse.ArgumentParser(
        description="Hub CLI - AI Agent Skills Hub Utility"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Compress command
    compress_parser = subparsers.add_parser(
        "compress", help="Compress a markdown file to save tokens"
    )
    compress_parser.add_argument("file", help="Path to the markdown file to compress")

    # Mode command
    mode_parser = subparsers.add_parser(
        "mode", help="Switch between operational modes (low/high)"
    )
    mode_parser.add_argument(
        "value",
        choices=["low", "high", "caveman", "premium"],
        help="The mode to switch to",
    )
    ...
    if args.command == "compress":
        ...
        if (
            state_file.exists() and state_file.stat().st_size > 5000
        ):  # Heuristic for ~1000 tokens
            print(
                f"STATE.md is large ({state_file.stat().st_size} bytes). Compressing..."
            )
            compressor = MarkdownCompressor()
