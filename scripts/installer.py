...
def install_remote_skill(name: str, target: Path, force: bool = False, repo_url: str = "https://github.com/KlebersonCollab/skills.git"):
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd_clone = ["git", "clone", "--filter=blob:none", "--sparse", repo_url, tmpdir]
            ...
            cmd_sparse = ["git", "-C", tmpdir, "sparse-checkout", "add", name, f".agents/skills/{name}", f".agent/skills/{name}"]
...
def install_local_skill(name: str, target: Path, force: bool = False):
    ...
    if not skill_to_install:
        ...
        print(f"💡 Dica: Tente usar --remote para buscar a skill no repositório oficial.")
        sys.exit(1)
...
def main():
    ...
    install_parser.add_argument("--remote", action="store_true", help="Faz o download diretamente do repositório remoto via Git Sparse-Checkout")
