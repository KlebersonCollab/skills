...
@patch("scripts.installer.get_all_skills")
def test_install_local_skill_not_found(
    mock_get_all_skills, mock_exit, mock_hub, tmp_path
):
...
@patch("scripts.installer.sys.exit")
@patch("scripts.installer.get_all_skills")
def test_install_local_skill_exists_no_force(
    mock_get_all_skills, mock_exit, mock_hub, tmp_path
):
    mock_exit.side_effect = SystemExit(1)
    skill_a = mock_hub / "skill-a"
...
def test_install_remote_skill_success(
    mock_safe_copytree, mock_subprocess_run, tmp_path, monkeypatch
):
    ...
    with patch(
        "scripts.installer.tempfile.TemporaryDirectory", return_value=MockTmpDir()
    ):
        install_remote_skill("skill-remote", target=tmp_path)
