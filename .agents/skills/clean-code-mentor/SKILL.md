...
1. **Continuous Audit**: Before suggesting any refactor or reviewing code, the agent **MUST** run **`hb mentor audit <file>`** to identify basic principle violations.
2. **Principle Mapping**: Identify which of the SOLID, YAGNI, DRY, or KISS principles are being violated.
3. **Educational Feedback**: Present the violation, the reason it's a concern, and the proposed clean solution.
