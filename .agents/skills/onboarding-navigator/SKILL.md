...
### Phase 3: 🛠️ Operational Protocol
1.  **Session Tracking**: Every new turn **MUST** begin with **`hb session start`** and end with **`hb session stop`**.
2.  **Strategic Alignment**: Run **`hb project update`** after completing feature tasks to keep the roadmap current.
3.  **Discovery**: Guide the user through the Hub by explaining the core skills and mandated CLI tools.
4.  **Culture Enforcement**: Ensure all agents follow the SDD workflow and architectural standards.
