...
1.  **Health Verification**: For every feature, the agent **MUST** run **`hb obs health <feature>`** to ensure observability sensors are defined in the contract.
2.  **Standardized Logging**: Use structured JSON logging for all components.
3.  **OpenTelemetry**: Ensure all services are instrumented with OTel for distributed tracing.
