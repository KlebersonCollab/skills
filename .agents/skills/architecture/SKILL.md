...
1.  **Decision Logging**: Every non-trivial change requires an ADR in `.specs/architecture/`.
2.  **Integrity Audit**: Use **`hb arch audit`** regularly to check if all features have their corresponding architectural justifications.
3.  **Visualization**: Use Mermaid for all architecture diagrams.
